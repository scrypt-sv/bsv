...
Interpreter.prototype.step = function () {
  ...
  function getSubScriptForCheckSig () {
    if (self.sighashScript) {
      // var preCount = 0
      // for(var i=0; i<this.pbegincodehash; i++) {
      //   var ck = this.script.chunks[i]
      //   if (ck && ck.opcodenum === Opcode.OP_CODESEPARATOR) {
      //     preCount += 1
      //   }
      // }

      // var count = 0, begin = 0
      // for(var i=0; i<this.sighashScript.chunks.length; i++) {
      //   var ck = this.sighashScript.chunks[i];
      //   if (ck && ck.opcodenum === Opcode.OP_CODESEPARATOR) {
      //     count += 1
      //   }
      //   if (count == preCount) {
      ...
      //   chunks: this.sighashScript.chunks.slice(begin)
      // })

      subscript = new Script().set({
        chunks: self.sighashScript.chunks
      })
    } else {
      // Subset of script starting at the most recent codeseparator
      // CScript scriptCode(pbegincodehash, pend);
      subscript = new Script().set({
        chunks: self.script.chunks.slice(self.pbegincodehash)
      })
    }
  ...
  if (fExec && opcodenum >= 0 && opcodenum <= Opcode.OP_PUSHDATA4) {
    ...
    switch (opcodenum) {
      ...
      case Opcode.OP_CHECKSIGVERIFY:
        ...
        subscript = getSubScriptForCheckSig()
